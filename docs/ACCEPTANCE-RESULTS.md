...
| Join and admission | Blocked | Requires deployed HTTPS callbacks and a real Google Meet. |
...
| Media reload/ack loss | Pass by implementation | Commands are claimed atomically and completed commands are not claimable again. |
...
| Webhook tampering | Pass | The runnable signature check rejects tampering and expired timestamps; deliveries and utterances have uniqueness constraints. |
